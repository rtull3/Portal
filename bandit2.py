	...
	if(statec=="on the floor"):
		...
		if (state == "released"):
			if (reddetect == 1):
				error = 300-targetpos
				if abs(error)>20:
					self.timed = time.time()-self.memtime
					self.memtime = time.time()
					rate = rate+ratedot*self.timed
					ratedot = error/1000
					self.msg.angular.z = -rate
					rospy.loginfo("Error[timed,rate,ratedot,error] " + str(self.timed)+ " " +str(rate) + "  " +str(ratedot) + " " + str(error))
					self.aim = 0
				else:
					rospy.loginfo("Sighted[timed,rate,ratedot,error] " + str(self.timed)+ " " +str(rate) + "  " +str(ratedot) + " " + str(error))
				...
				self.msg.linear.x = 0
				rospy.loginfo("All sectors blocked doing full rotation")
			else:
