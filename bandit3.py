...
class Scan_msg:
	...
	global reddetect, targetpos, soundtimer
    ...
    def backup(self, duration, weight):
	    ...
	    while (rospy.get_time() < stopTime):
		 ...
		 self.pub.publish(twist)
		 rospy.sleep(.5)
	    playsound("searching.wav",100)
    def shoot(self,fpos):
	global trigger
	playsound("alert.wav",0)
	playsound("alert.wav",0)
...
def playsound(sname,time):
	global p, soundtimer
	if soundtimer == 0:
		#define stream chunk   
		chunk = 1024  
		filename = "/home/turtlebot/portal/sounds/"+str(sname)
		soundtimer = time
